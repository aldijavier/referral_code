var gymie = (function ($) {
	...
	return {
		...
		/* Rules Details
		/* --------------------------------- */
		chooserules: function () {

			$("#corporate_rules").hide();
			$("#broadband_rules").hide();
			

			$('#service_id').on('change', function () {
				if (this.value == '0') {
					$("#corporate_rules").show();
					$("#broadband_rules").hide();

				}else if(this.value == '1'){
					$("#broadband_rules").show();
					$("#corporate_rules").hide();
				}
			});

			$('#ruletype_id').on('change', function () {
				if (this.value == '0') {
					$("#subscription").show();
					$("#revenue").hide();
		...
		loadmorris1: function () {
			...
			var donut1 = new Morris.Donut({
				element: 'gymie-referral-per-plan',
				resize: true,
				colors: ["#e74c3c", "#e67e22", "#3498db"],
				data: JSON.parse(jsReferralPerPlan),
				hideHover: 'auto'
			});

			// LINE CHART
			var line = new Morris.Line({
				element: 'gymie-referral-trend',
				resize: true,
				data: JSON.parse(jsReferral),
				xkey: 'month',
				ykeys: ['referral_code'],
				labels: ['Reedem Code'],
				hideHover: 'auto',
		...
		loadmorris2: function () {
			...
			//DONUT CHART
			var donut = new Morris.Donut({
				element: 'gymie-referralagent-per-plan',
				resize: true,
				colors: ["#e74c3c", "#e67e22", "#3498db"],
				data: JSON.parse(jsReferralPerPlanagent),
				hideHover: 'auto'
			});

			// LINE CHART
			var line = new Morris.Line({
				element: 'gymie-referralagent-trend',
				resize: true,
				data: JSON.parse(jsReferralagent),
				xkey: 'month',
				ykeys: ['referral_code'],
				labels: ['Reedem Code'],
				hideHover: 'auto',
		...
		calcTotal: function(){
			...
			var serviceqty = parseInt($("#serviceqty").val());
			var servicefee = parseInt($("#servicefee").val());

				// var serviceqty = parseInt($("#serviceqty").val()); // Get Selected plan amount
				// var servicefee = $("#servicefee").length ? parseInt($("#servicefee").val()) : 0; // Get selected admission amount
				var total_price = serviceqty * servicefee;
